...
module Dea
  class StatCollector
    ...
    def emit_metrics(now)
      ...
      pcpu = compute_cpu_usage(info.cpu_stat.usage,now) || 0
      used_memory = compute_memory_usage(info.memory_stat) || 0
      used_disk = info.disk_stat ? info.disk_stat.bytes_used : 0

      Dea::Loggregator.emit_container_metric(
        @application_id, @instance_index, pcpu, used_memory, used_disk)
    end
    ...
    def stop_stat_collector
     ...
     def run_stat_collector
       Promise.resolve(promise_emit_stats(Time.now)) do
