  ...
  class VarzCollector
    class UsageSnapshot
      ...
      def self.for_instance(instance)
        # While memory is reported in kB, disk is reported in bytes :(.
        new(:used_memory     => instance.used_memory,
            :reserved_memory => instance.attributes["limits"]["mem"] / 1024,
    ...
    def update
      ...
      hashify = proc { |k, v| [k, v.to_hash] }
      VCAP::Component.varz[:frameworks] = Hash[by_framework.map(&hashify)]
      VCAP::Component.varz[:runtimes] = Hash[by_runtime.map(&hashify)]

      # Instance listing
      VCAP::Component.varz[:running_apps] =
        bootstrap.instance_registry.map { |i| snapshot_instance(i) }
    ...
    def snapshot_instance(instance)
      { :droplet_id      => instance.application_id,
        ...
        :usage           => UsageSnapshot.for_instance(instance).to_hash,
