  ...
  describe "health checks" do
    ...
    describe "#promise_read_instance_manifest" do
      it "delivers {} if no container path is returned" do
        stub_container_path(instance, nil)

        instance.promise_read_instance_manifest.resolve.should == {}
      end

      it "delivers {} if the manifest path doesn't exist" do
        stub_container_path(instance, tmpdir)
      ...
      it "delivers the parsed manifest if the path exists" do
        ...
        instance.promise_read_instance_manifest.resolve.should == manifest
      end

      def stub_container_path(instance, path)
        info_reply = mock("info_reply")
        info_reply.stub(:container_path).and_return(path)

        p = delivering_promise(info_reply)
        instance.stub(:promise_container_info).and_return(p)
      end
    end
  end
end
