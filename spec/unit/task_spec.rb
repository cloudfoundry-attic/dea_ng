...
describe Dea::Task do
  ...
  describe "#promise_warden_call_with_retry" do
    ...
    it "succeeds when container#call succeeds" do
      task.container.
        should_receive(:call).
        with(:name, request).
        and_return("ok")
    ...
    it "fails when container#call fails with ::EM::Warden::Client::Error" do
      task.container.
        should_receive(:call).
        with(:name, request).
        and_raise(::EM::Warden::Client::Error.new("error"))

      expect_failure
    end

    it "retries when container#call fails with ::EM::Warden::Client::ConnectionError" do
      task.container.
        should_receive(:call).
        with(:name, request).
        ordered.
        and_raise(::EM::Warden::Client::ConnectionError.new("error"))
