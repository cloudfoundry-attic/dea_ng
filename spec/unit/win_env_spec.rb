...
describe Dea::WinEnv do
  ...
  let(:environment) { ["A=one_value", "B=with spaces", "C=with'quotes\"double", "D=referencing + $env:A", "E=with=equals", "F="] }
  let(:debug) { nil }
  ...
  subject(:env) { Dea::WinEnv.new(StartMessage.new(starting_message), instance) }

  def self.it_exports(name, value)
    it "exports $#{name} as #{value}" do
      stub_const('VCAP::WINDOWS', true)
      example = "$env:%s='%s'" % [name, value]
      expect(exported_variables).to include example
    end
  end

  def self.it_does_not_exports(name, value)
    it "exports $#{name} as #{value}" do
      stub_const('VCAP::WINDOWS', true)
      example = "$env:%s='%s'" % [name, value]
      expect(exported_variables).to_not include example
    end
  end
  ...
  context "when running from the instance task" do
    ...
    describe "#exported_system_environment_variables" do
      ...
      it_does_not_exports "VCAP_DEBUG_IP", ""
      it_does_not_exports "VCAP_DEBUG_PORT", ""
      it_exports "PORT", "$env:VCAP_APP_PORT"
      ...
      context "when it does NOT have a DB" do
        ...
        it_does_not_exports "DATABASE_URL", ""
      end
    end
    ...
    describe "#exported_user_environment_variables" do
      ...
      it_exports "D", "referencing + $env:A"
  ...
  context "when running from the staging task" do
    ...
    subject(:env) { Dea::WinEnv.new(StagingMessage.new(staging_message), staging_task) }
